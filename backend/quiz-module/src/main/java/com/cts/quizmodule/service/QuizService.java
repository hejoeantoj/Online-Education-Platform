...
public class QuizService {
    ...
    public Quiz createQuiz(QuizDTO quizDTO) throws DuplicateQuizException,CourseNotFoundException,InstructorNotAllowedException{
        ...
        List<Quiz> quizList=quizDao.findByCourseId(quizDTO.getCourseId().toString());
        
        for(Quiz list:quizList) {
        	if (list.getTitle().equalsIgnoreCase(quizDTO.getTitle())) {
        	    throw new DuplicateQuizException("Title Already exists");
        	}
