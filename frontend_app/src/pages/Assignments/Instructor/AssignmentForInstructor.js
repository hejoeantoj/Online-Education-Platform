...
import styles from '../css/AssignmentForInstructor.module.css'; // Import CSS Module
...
const AssignmentForInstructor = () => {
    ...
    const handleViewSubmissions = async (assignmentId) => {
        ...
        try {
            ...
            if (response.data && response.data.success && Array.isArray(response.data.data)) {
                ...
                // Only show submissions that haven't been marked yet in the "View Submissions" modal
                ...
                setSubmissions([]); // Ensure submissions is an empty array on failure/no data
                console.error('Unexpected response structure for submissions:', response.data);
        ...
        try {
            const response = await axios.get(`http://localhost:8083/api/asubmission/viewFiles`, {
                params: {
                    ...
                    instructorId: instructorUuid // Assuming API needs instructorId for authorization check
                },
                headers: {
                    Authorization: `Bearer ${token}`
                },
            ...
            let filename = `submission_student_${studentId}_assignment_${assignmentId}.pdf`; // Default filename
            ...
            if (error.response && error.response.status === 404) {
                 ...
                 // Try to read error message from blob response if backend sends one
                try {
    ...
    const handleAssignMarks = (submission) => {
        ...
        setIsAssignMarksModalOpen(true); // Open the assign marks modal
        setIsSubmissionModalOpen(false); // Close the view submissions modal
    };
 
     // Called when marks are successfully assigned in the modal
     const handleMarksAssigned = (assignmentId) => {
        fetchAssignments(); // Re-fetch assignments (might update counts or status later)
        // Re-fetch submissions for the *currently selected assignment* to update the list
        // Use selectedAssignment.assignmentId if available, otherwise use the passed assignmentId
        ...
        // Optionally, keep the Assign Marks modal open or close it based on UX preference
         // setIsAssignMarksModalOpen(false); // Close assign marks modal after success
    ...
    return (
        <div className={styles.container}>
            ...
            <ViewSubmissionsModal
                ...
                onAssignMarks={handleAssignMarks} // Pass handler to open assign marks modal
            />
             ...
             <AssignMarksModal
                ...
                onRequestClose={() => {
                    ...
                    // Optional: Re-open the submission list modal if needed
                    // if (selectedAssignment) {
                    //     handleViewSubmissions(selectedAssignment.assignmentId);
                    // }
                 }}
                selectedSubmission={selectedSubmission}
                selectedAssignment={selectedAssignment} // Pass assignment for context (total marks)
                token={token}
                instructorUuid={instructorUuid}
